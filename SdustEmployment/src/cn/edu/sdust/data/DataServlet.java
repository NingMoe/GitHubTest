...
public class DataServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag.equals("main")) {
			...
			int countPage = DbUtils.getCountPage("RelatedData");
			int count = DbUtils.getCountRecord("RelatedData");
			request.getSession().setAttribute("countPage", countPage);
			request.getSession().setAttribute("count", count);
			request.getSession().setAttribute("countPageNow", 1);
			
			DataInterface di = new DataImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = di.getAllRDData(1);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/ziliao.jsp").forward(
					request, response);
			...
			DataInterface di = new DataImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = di.getAllRDData(countPageNow);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/ziliao.jsp").forward(
					request, response);
			...
			DataInterface di = new DataImpl();
			StuQuery stuQuery = di.getAllRDDataByID(ID);
			request.setAttribute("stuQuery", stuQuery);
			request.getRequestDispatcher("../Career/ziliao2.jsp").forward(
					request, response);
		}
		out.flush();
		out.close();
