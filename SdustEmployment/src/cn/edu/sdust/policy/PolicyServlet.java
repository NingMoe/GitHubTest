...
public class PolicyServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag.equals("main")) {
			...
			int countPage = DbUtils.getCountPage("EmpPolicy");
			int count = DbUtils.getCountRecord("EmpPolicy");
			request.getSession().setAttribute("countPage", countPage);
			request.getSession().setAttribute("count", count);
			request.getSession().setAttribute("countPageNow", 1);
			PolicyInterface pi = new PolicyImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = pi.getAllEmpPolicy(1);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/zhengce.jsp").forward(
					request, response);
			...
			PolicyInterface pi = new PolicyImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = pi.getAllEmpPolicy(countPageNow);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/zhengce.jsp").forward(
					request, response);
			...
			PolicyInterface pi = new PolicyImpl();
			StuQuery stuQuery = pi.getAllEmpPolicyByID(ID);
			request.setAttribute("stuQuery", stuQuery);
			request.getRequestDispatcher("../Career/zhengce2.jsp").forward(
					request, response);
		}
