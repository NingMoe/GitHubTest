...
public class DynamicServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag.equals("main")) {
			int countPage = DbUtils.getCountPage("EmpDynamic");
			int count = DbUtils.getCountRecord("EmpDynamic");
			request.getSession().setAttribute("countPage", countPage);
			request.getSession().setAttribute("count", count);
			request.getSession().setAttribute("countPageNow", 1);
			DynamicInterface di = new DynamicImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = di.getAllEmpDynamic(1);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/dongtai.jsp").forward(
					request, response);
			...
			DynamicInterface di = new DynamicImpl();
			List<StuQuery> stuQuerys = new ArrayList<StuQuery>();
			stuQuerys = di.getAllEmpDynamic(countPageNow);
			request.setAttribute("stuQuerys", stuQuerys);
			request.getRequestDispatcher("../Career/dongtai.jsp").forward(
					request, response);
			...
			DynamicInterface di = new DynamicImpl();
			StuQuery stuQuery = di.getAllEmpDynamicByID(ID);
			request.setAttribute("stuQuery", stuQuery);
			request.getRequestDispatcher("../Career/dongtai2.jsp").forward(
					request, response);
		}
		out.flush();
		out.close();
